...
setwd("~/Milou - Projects/GRH-1/SWI/2019-07-26")
Molt <- read.csv('2019-07-26 MoltAnalysis.csv')
GFP <- read.csv('2019-09-22 Threshold 07/2019-07-26 GFP intensities T07.csv')
GFP_BGsub <- GFP[,-3]
tp <- 100
strain1 <- 'Pqua-1'
...
plot(GFP_strain1_eth$meanGFP, type='l', main=strain1, ylim=c(0,500), xaxt='n'); lines(GFP_strain1_aux$meanGFP, col='red'); lines(GFP_strain1_eth$upperGFP, col='grey'); lines(GFP_strain1_eth$lowerGFP, col='grey'); lines(GFP_strain1_aux$upperGFP, col='pink'); lines(GFP_strain1_aux$lowerGFP, col='pink'); axis(1, at = 1:tp, labels = colnames(GFP_strain1_eth$gfp), cex.axis = 0.7); abline(v=mean(infoMolt$M3entry_M2exit[which(infoMolt$condition=='ethanol' & infoMolt$strain=='Pqua-1')])); abline(v=mean(infoMolt$M3exit_M2exit[which(infoMolt$condition=='ethanol' & infoMolt$strain=='Pqua-1')])); abline(v=mean(infoMolt$M3entry_M2exit[which(infoMolt$condition=='auxin' & infoMolt$strain=='Pqua-1' & infoMolt$phen.descrip=='L3 phenotype')]), col='red')
...
matplot(t(GFP_strain1_eth$gfp_M2exit), ylim=c(0,500),type='l', lty=1, col='grey', main=GFP_strain1_eth$name,xaxt = 'n', ylab='background subtracted GFP intensity', xlab='time after M2 exit (h)'); matplot(t(GFP_strain1_eth$gfp_molt), type='l', lty=2, col=alpha('blue',0.5), add=TRUE); lines(GFP_strain1_eth$meanGFP);  lines(GFP_strain1_eth$upperGFP, col='red', lty=2); lines(GFP_strain1_eth$lowerGFP, col='red', lty=2); axis(1, at = 1:tp, labels = colnames(GFP_strain1_eth$gfp), cex.axis = 0.7) 
matplot(t(GFP_strain1_aux$gfp_M2exit), ylim=c(0,500),type='l', lty=1, col='grey', main=GFP_strain1_aux$name,xaxt = 'n', ylab='background subtracted GFP intensity', xlab='time after M2 exit (h)'); matplot(t(GFP_strain1_aux$gfp_molt_phenotype), type='l', lty=2, col=alpha('blue',0.5), add=TRUE); lines(GFP_strain1_aux$meanGFP);  lines(GFP_strain1_aux$upperGFP, col='red', lty=2); lines(GFP_strain1_aux$lowerGFP, col='red', lty=2); axis(1, at = 1:tp, labels = colnames(GFP_strain1_aux$gfp), cex.axis = 0.7) 
#matplot(t(GFP_strain2_eth$gfp_M2exit), type='l', lty=1, col='grey', main=GFP_strain2_eth$name,xaxt = 'n', ylab='background subtracted GFP intensity', xlab='time after M2 exit (h)'); matplot(t(GFP_strain2_eth$gfp_molt), type='l', lty=2, col=alpha('blue',0.5), add=TRUE); lines(GFP_strain2_eth$meanGFP);  lines(GFP_strain2_eth$upperGFP, col='red', lty=2); lines(GFP_strain2_eth$lowerGFP, col='red', lty=2); axis(1, at = 1:tp, labels = colnames(GFP_strain2_eth$gfp), cex.axis = 0.7) 
#matplot(t(GFP_strain2_aux$gfp_M2exit), type='l', lty=1, col='grey', main=GFP_strain2_aux$name,xaxt = 'n', ylab='background subtracted GFP intensity', xlab='time after M2 exit (h)'); matplot(t(GFP_strain2_aux$gfp_molt_phenotype), type='l', lty=2, col=alpha('blue',0.5), add=TRUE); lines(GFP_strain2_aux$meanGFP);  lines(GFP_strain2_aux$upperGFP, col='red', lty=2); lines(GFP_strain2_aux$lowerGFP, col='red', lty=2); axis(1, at = 1:tp, labels = colnames(GFP_strain2_aux$gfp), cex.axis = 0.7) 
...
mean(infoMolt$M3entry_M2exit[which(infoMolt$condition=='ethanol' & infoMolt$strain=='Pqua-1')])/6
#7.989583
mean(infoMolt$M3exit_M2exit[which(infoMolt$condition=='ethanol' & infoMolt$strain=='Pqua-1')])/6
#10.17708
